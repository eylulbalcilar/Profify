...
app.post("/documents", auth, async (req, res) => {
    try {
        ...
        res.status(201).json({ 
            status: "Created",
            message: `Document for ${studentName} has been successfully registered.`
        });
    } catch (e) {
        res.status(500).json({ error: "Internal server error" });
    }
    ...
    try {
        ...
        if (item.ownerOrg !== req.orgID) {
            return res.status(403).json({ 
                error: "Forbidden",
                message: "Access denied: This document belongs to another organization." 
            });
        }
        res.status(202).json({ 
            status: "Accepted",
            message: `Verification successful: ${item.studentName}'s record is valid.`
        });
    } catch (e) {
        res.status(500).json({ error: "Internal server error" });
    }
...
app.delete("/documents/:id", auth, async (req, res) => {
    try {
        ...
        res.status(200).json({ 
            message: "Document successfully removed from the system." 
        });
    } catch (e) {
        res.status(500).json({ error: "Internal server error" });
    }
